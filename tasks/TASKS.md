...
| T080 | FluxMotionText full UI Text Animators — dynamic Add Animator workflow, visible animator stack, selector shapes, range controls, per-character/word/line renderer evaluation, playback/cache invalidation, and direct Add Animator UX are manually accepted by Nick as 100% working for the core task. Remaining polish is tracked separately in T081 and the recovery source of truth. | DONE | opencode | 2026-05-25 | 2026-05-26 | tasks/FLUX_MOTION_TEXT_RECOVERY_SOURCE_OF_TRUTH.md |
| T081 | Text Animator finishing touches — preserve target values when moving keyframes/changing selector shape; add selectable transform anchor dropdown; add sliders for Scale/Start/End/Offset/Strength; selecting animator timeline rows opens Animator panel and outlines selected animator. Implementation/build passed; GUI validation pending. | REVIEW | opencode | 2026-05-26 | — | tasks/FLUX_MOTION_TEXT_RECOVERY_SOURCE_OF_TRUTH.md |
...
| T083 | AI matte/mask generation and video depth tools — add AI-assisted matte generation/extraction with multiple models and a model-choice architecture. Initial candidates include SAM 3.1, MatAnyone/MatAnything-style video matting models, BFRNet or similar restoration/segmentation helpers where useful, plus other strong video mask generation/extraction models. Also research and integrate video-stable depth estimation; do not assume Depth Anything is sufficient because Nick flagged it as weak for video. Must research current best models online and local integration constraints before implementation. | PENDING | opencode | — | — | — |
...
| ID | Task | Status | Assigned | Started | Completed | File |
|---|---|---|---|---|---|---|
| T070 | Linux workstation installer/checker — Fedora-first path-agnostic `--bootstrap` helper derives `$FLUX_ROOT`, supports `$BUILD_DIR`/`$PLUGIN_PREFIX`/`$OFX_USER_PLUGIN_DIR`, updates submodules, verifies/installs deps, configures/builds Flux, bootstraps embedded-Python deps, deploys PyPlugs/OFX bundles, writes user launcher, clears scoped OFX cache, stages transferable OFX extras, and validates OFX binaries with `ldd`; canonical Linux runbook added. Needs clean Fedora VM/container or distrobox end-to-end validation before DONE. | TESTING | forge | 2026-05-24 | — | tasks/T070-linux-workstation-installer.md |
