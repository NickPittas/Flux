...
| T051 | Adjustment row trim/split — keyframe-based enable/disable on each effect's disable knob. Trim start: keyframe disabled at startFrame-1, enabled at startFrame. Trim end: keyframe disabled at endFrame+1. All effects in the adjustment row must be keyframed simultaneously. Split duplicates the row then trims both halves. Requires per-effect disable knob animation management on every trim change. | PENDING | — | — | — | Gui/FluxTimeline.{h,cpp}, Gui/Gui05.cpp |
