...
- Validation: tracking/documentation edits only. T073 source validation already passed via `git diff --check -- Gui/DopeSheetView.cpp` and `cmake --build build --target Natron`; T085 final preserved baseline remains `build-logs/flux-warning-pass4-20260526-160824.warnings.txt` with 32 warning lines.
