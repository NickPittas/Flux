...
### 2026-05-26 — FluxMotionText/TextRender viewer zoom render-scale bug fixed

- What was done: debugged the user-reported FluxMotionText viewer zoom failure where changing Viewer zoom caused TextRender output to resize, crop, or corrupt cached tiles until app restart. The final accepted fix was made in the Flux-owned TextRender path and then deployed to the installed Flux plugin bundle.
- What was found: TextRender originally used the fetched destination bounds as the layout canvas and later incorrectly forced full 1920x1080 output at half render scale while the host destination was 960x540. The decisive breadcrumb was the temporary bounds log: `rs=(0.500,0.500) renderWindow=(0,0,960,540) dst=(0,0,960,540) rod=(0,0,1920,1080)`. The correct invariant is: destination/output bounds come from the host image, while layout bounds come from project/RoD multiplied by render scale.
- What changed: `openfx-flux/TextRender/TextRender.cpp` now passes `args.renderScale` into `RenderRequest`, sets `outputBounds` from `dst->getBounds()`, and computes stable `layoutBounds` from RoD/project bounds scaled by render scale. `openfx-flux/TextRender/TextRasterizer.*` separates layout bounds from output bounds, allocates/writes only the output image, centers/layouts text against stable scaled project bounds, and scales font/tracking/leading/animator offsets consistently. Temporary `[DBG-TextRender-bounds]` logging was removed after the fix was validated.
- Why: Viewer zoom/proxy must never change text layout. TextRender must produce the requested project/RoD-space image+alpha for the host-provided render scale and bounds; the Viewer then displays/scales the finished image.
- Validation: `git diff --check -- openfx-flux/TextRender/TextRender.cpp openfx-flux/TextRender/TextRasterizer.h openfx-flux/TextRender/TextRasterizer.cpp` passed; `cmake --build build/openfx-flux -- -j$(nproc)` passed; the built `build/openfx-flux/FluxTextRender.ofx` was copied into `plugins/FluxTextRender.ofx.bundle/Contents/Linux-x86-64/FluxTextRender.ofx`; `./tools/linux/flux-linux-setup.sh --deploy-extras --clear-ofx-cache --force` redeployed the installed plugin; `sha256sum` confirmed the build and installed plugin binaries matched. Nick manually validated the canonical GUI behavior and reported: “it works now perfectly.”
- Failures: several earlier attempts failed because they were tested against an old installed plugin binary; `--deploy-extras` copies from the repo bundle, so after direct CMake builds the built `FluxTextRender.ofx` must be copied into `plugins/FluxTextRender.ofx.bundle` before deployment unless the installer `--build` path is used. This deployment rule was documented in `INSTALL_FLUX_LINUX.md`.
- Next step: keep TextRender zoom/proxy behavior as accepted. Future TextRender work should preserve the layout/output-bounds split and verify build-vs-installed plugin hashes before GUI validation.

### 2026-05-26 — Batch task triage after T073/T085 follow-up

- What was done: recorded Nick's decisions for the small follow-up batch covering T072, T073, T084, and T085.
- What was found: T072's native Text alignment issue belongs to external openfx-arena `Text.ofx.bundle`; the source is not in this repo and the Flux product direction is the Flux-owned `TextRender`/FluxMotionText path. T073 paint-only Dope Sheet polish was implemented in `Gui/DopeSheetView.cpp`, built, reviewed, and accepted by Nick as done. T085 warning audit reduced the clean warning baseline from 908 to 32 warning lines; the remaining warnings are accepted residual generated/third-party/Python-compat noise for this pass. T084 deploy logic is implemented; Nick later accepted moving T084 to DONE from the recorded local validation evidence.
- What changed: `tasks/TASKS.md` marks T072 `CANCELLED`, T073 `DONE`, and T085 `DONE`; `plans/PHASES.md` records the same status decisions; `tasks/T085-build-warning-audit.md` now records closure and accepted residual warning baseline.
- Why: Avoid spending Flux product time patching an external legacy native Text provider while the accepted path is FluxMotionText/TextRender; close completed polish/audit work without pretending T084's environment validation passed.
- Validation: tracking/documentation edits only. T073 source validation already passed via `git diff --check -- Gui/DopeSheetView.cpp` and `cmake --build build --target Natron`; T085 final baseline was 32 warning lines. The transient `build-logs/` captures were removed from the tracked tree during repository cleanup and remain recoverable from git history.
- Failures: T084 full `--check --validate-ldd --validate-ofx-discovery` exited nonzero on the current host during the earlier batch because Fedora development packages were reported missing and the OFX cache XML was not regenerated yet, even though ldd and cold-cache TextRender discovery passed. Nick subsequently accepted closing T084 anyway.
- Next step: T084 is now DONE by Nick's instruction; continue to T082 Illustrator import or T083 AI matte/depth when Nick chooses.
