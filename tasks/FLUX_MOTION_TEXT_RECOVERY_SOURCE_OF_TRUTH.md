...
- Failures: T084 full `--check --validate-ldd --validate-ofx-discovery` still exits nonzero on the current host because Fedora development packages are reported missing and the OFX cache XML is not regenerated yet, even though ldd and cold-cache TextRender discovery pass.
- Next step: keep T084 `IN_PROGRESS` until a clean/fresh validation run passes; otherwise continue to T082 Illustrator import or T083 AI matte/depth when Nick chooses.
