# -*- coding: utf-8 -*-
# FluxSolid PyPlug — wraps Constant → FrameRange → TimeOffset → Transform → Grade → Output
# Solid color layer (opaque, project-format-sized fill).
#
# Stable parameter contract (C++ uses these exact names via getKnobByName):
#   color        (RGB — aliased to Constant's "color")
#   frameRange   (Int2D: first, last)
#   timeOffset   (Int)
#   opacity      (RGBA — aliased to Grade multiply, all 4 dims set uniformly for opacity)
#   translate    (Double2D)
#   scale        (Double2D)
#   uniform      (Boolean)
#   rotate       (Double)
...
def getPluginID():
    return "net.sf.openfx.FluxSolid"
...
def getDescription():
    return "Flux solid layer: Constant -> FrameRange -> TimeOffset -> Transform -> Output"
...
def createInstance(app, group):
    ...
    page = group.createPageParam("SolidSettings", "Solid Settings")

    # -- Color (RGBA) --
    colorParam = group.createColorParam("color", "Color", True)  # True = RGBA
    colorParam.setDisplayMinimum(0, 0)
    colorParam.setDisplayMaximum(1, 0)
    colorParam.setDisplayMinimum(0, 1)
    colorParam.setDisplayMaximum(1, 1)
    colorParam.setDisplayMinimum(0, 2)
    colorParam.setDisplayMaximum(1, 2)
    colorParam.setDisplayMinimum(0, 3)
    colorParam.setDisplayMaximum(1, 3)
    colorParam.setDefaultValue(0, 0)
    colorParam.setDefaultValue(0, 1)
    colorParam.setDefaultValue(0, 2)
    colorParam.setDefaultValue(1, 3)  # alpha = 1 (fully opaque)
    colorParam.restoreDefaultValue(0)
    colorParam.restoreDefaultValue(1)
    colorParam.restoreDefaultValue(2)
    colorParam.restoreDefaultValue(3)
    colorParam.setAnimationEnabled(True)
    page.addParam(colorParam)
    ...
    # -- Opacity (aliased to Grade multiply, RGBA) --
    opacityParam = group.createColorParam("opacity", "Opacity", True)  # True = RGBA
    ...
    group.setPagesOrder(["SolidSettings", "Node", "Settings"])
    ...
    # Constant node (generates solid color at project format, RGBA with alpha)
    constantNode = app.createNode("net.sf.openfx.ConstantPlugin", 1, group)
    constantNode.setScriptName("Constant1")
    constantNode.setLabel("Constant1")
    constantNode.setPosition(0, -200)
    constantNode.setSize(130, 79)
    ...
    # Enable all 4 channels so multiply affects RGBA (processA defaults to false)
    ...
    # ====================================================================
    # 3. Connect: Constant → FrameRange → TimeOffset → Transform → Multiply → Output
    # ====================================================================

    frameRangeNode.connectInput(0, constantNode)
    ...
    # Color: alias group "color" -> Constant1 "color"
    group.getParam("color").setAsAlias(constantNode.getParam("color"))
    ...
    # Before/After: alias to FrameRange1 "before"/"after"
    ...
    # Opacity: alias group "opacity" -> Multiply1 "value" (RGBA)
